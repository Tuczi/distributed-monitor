...
void distributed_mutex::request() {
	...
	clock.update();
	request_ts = clock;
	p_monitor->proxy.broadcast(mpi_serial_t(mpi_serial_t::type_t::REQUEST, resource_id, request_ts), p_monitor->tag, p_monitor->comm);
}
...
void distributed_mutex::response() {
	...
	for(int i=0; i<p_monitor->comm.Get_size(); i++) {
		if(waiting_for_respose[i]) {
			...
			p_monitor->proxy.send(mpi_serial_t(mpi_serial_t::type_t::RESPONSE, resource_id, request_ts), i, p_monitor->tag, p_monitor->comm);
		}
	}
